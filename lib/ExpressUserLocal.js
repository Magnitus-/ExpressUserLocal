...
ValidationRoutes['UsersPOST'] = function(UserSchema, EmailTokenGen, SendEmail) {
    ...
    return(function(Req, Res, Next) {
        ...
        {
            EmailTokenGen(function(Err, EmailToken) {
                if(Err)
                {
                    Next(Err);
                    return;
                }
                if((EmailAuthFields.length>0)&&SendEmail)
                {
                    Locals.ExpressUser['User'][EmailAuthFields[0]] = EmailToken;
                    SendEmail(Locals.ExpressUser['User'], EmailToken, function(Err) {
                        Next(Err);
                    });
                }
                else
...
module.exports = function(Options)
{
    ...
    var EmailTokenGen = Options && Options.EmailTokenGen ? Options.EmailTokenGen : function(Callback) {Callback(null, Uid(20));};
    ...
    return function(Router, Roles) {
        ...
        Router.post('/Users', ValidationRoutes.UsersPOST(UserSchema, EmailTokenGen, SendEmail));
